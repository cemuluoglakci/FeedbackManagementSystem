...
{
    public class DirectFeedbackCommandHandler : IRequestHandler<DirectFeedbackCommand, BaseResponse>
    {
        ...
        public async Task<BaseResponse> Handle(DirectFeedbackCommand request, CancellationToken cancellationToken)
        {
            ...
            {
                return new BaseResponse(0, "Only related company representatives can direct feedbacks.");
            }

            User companyEmployee = await _context.User.Include(x => x.Role).FirstOrDefaultAsync(x => x.Id == request.EmployeeId);
            if (companyEmployee == null)
            {
                return new BaseResponse(0, "Company employee was not found.");
            }
            if (companyEmployee.CompanyId != feedback.CompanyId || companyEmployee.Role.RoleName != Constants.CompanyEmployeeRole)
            {
                return new BaseResponse(0, "User is not authorized to reply feedbacks in the name of the company");
            }
